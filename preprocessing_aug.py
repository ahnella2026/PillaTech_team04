...
def main() -> None:
    ...
    print(f"\n[7] Applying augmentation for: {EXP_NAME}")
    
    if EXP_NAME == "oversampling":
        # 3번 방법: 이미지 이름만 리스트에서 복사
        train_images = oversample_rare_class_images(train_images, image_meta, duplication_factor=2)
        print(f"    - Oversampling complete. Total train count: {len(train_images)}")

    elif EXP_NAME == "copy_paste_v1":
        # 1번 방법: 새로운 합성 이미지 생성
        # merged['images'] 딕셔너리를 직접 전달해야 함에 유의
        aug_annots = apply_copy_paste_augmentation(train_images, merged, rare_classes, target_count=20)
        
        # 메타데이터 업데이트
        merged.update(aug_annots)
        
        # 학습 리스트에 합성된 이미지 이름 추가
        train_images.extend(aug_annots.keys())
        print(f"    - Copy-Paste V1 complete. Added {len(aug_annots)} images.")
