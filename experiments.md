...
| **Exp 10(F)**| **3-Seed Ens**| Exp 10-1~3 WBF 앙상블 | v11s | 960 | - | **0.9942** | **0.9927** | **0.9942** | - | `metrics/exp10_final_ensemble_metrics.json` 기준 최종 val 앙상블 결과 |
...
*   캐글 제출용 CSV는 별도 추론 스크립트 `src/test_custom_v5.py`를 사용해 `test_images` 기준으로 생성함.
...
*   **재현(추론)**: `python src/test_custom_v5.py --model runs/pill_exp5_yolo11s_copypaste/weights/best.pt --imgsz 1024 --conf 0.20 --iou 0.60 --output submission/exp8_submission_iou0.6.csv`
...
**1. 해상도별 추론 단계의 NMS (`test_custom_v5.py`)**
...
### [Exp 10] 3-Seed Ensemble Training
*   **배경**: 동일 아키텍처 내 파라미터 튜닝(Exp 8, 9)은 성능 향상이 임계점에 도달함. 모델의 우연성(Variance)을 줄이고 일반화 성능을 확보하기 위해 3-Seed(42, 123, 777) 실험을 진행함.
*   **실험 결과**:
...
*   **통찰**: 3개 시드의 평균 mAP50-95는 **0.9926 (±0.0019)** 로 매우 높은 안정성을 보였고, 시드 42가 개별 최고 성능(0.9942)을 기록함. 실무에서는 3개 시드를 통해 검증하는 작업이 중요하지만 한정된 시간 안에 캐글 점수 올리는 데는 불필요한 작업이었던 것 같음. 
...
*   **결정**: 개별 seed 평균과 비교해 WBF 앙상블 이득은 크지 않았지만, 제출용 최종 파일 `submission/exp10_final_submission.csv` 를 생성해 캐글 점수로 최종 검증을 진행함.  
...
4.  **`test_custom_v5.py`** 실행 (추론 + Config 자동 반영)
    * `python src/test_custom_v5.py --config configs/exp8_inference.yaml`
