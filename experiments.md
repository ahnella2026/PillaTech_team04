...
| **Exp 5** | **Copy-Paste** | Stratified + Custom Copy-Paste | **v11s** | 960 | 50 | **0.995** | **0.993** | **0.995** | **50** | **Kaggle: 0.96804(성능 상승)** |
| **Exp 6** | **Rotation Aug** | Stratified / Flip Off + Degree 180 + CP | **v11s** | 960 | 50 | 0.990 | 0.940 | 0.995 | 50 | **Kaggle: 0.85483 (성능 급락)** |
| **Exp 7** | **Final High-Res** | Stratified / Flip Off + CP (No Rotation) | **v11s** | 1024| 50 | **0.995** | **0.993** | **0.995** | **50** | **성능 정체 (해상도 리사이즈 한계 확인)** |
| **Exp 8** | **Dynamic NMS Tuning** | Exp 5 기반 Validation 셋 최적 conf/iou 탐색 | **v11s** | 960 | - | - | - | - | - | **진행 예정 (1순위 - 최우선, 하드코딩 제거)** |
| **Exp 9** | **Snapshot Ensemble** | Exp 5의 Best Epoch(45,48,50) WBF 결합 | **v11s** | 960 | - | - | - | - | - | **진행 예정 (1순위 - 리소스 소모 0)** |
| **Exp 10** | **Multi-scale WBF** | 해상도 3종(800, 960, 1024) 추론 결합 | **v11s** | Mix | - | - | - | - | - | **진행 예정 (1순위 - 모델 한계치 도달)** |
| **Exp 11** | **3-Seed Ensemble** | Seed 다변화(42,123,777) 훈련 후 평균 앙상블 | **v11s** | 960 | 50 | - | - | - | - | **진행 예정 (2순위 - 병렬학습, 분산 파악)** |
| **Exp 12** | **YOLO26-n Baseline** | 2026년 최신 NMS-free 아키텍처 적용 | **v26n** | 960 | 50 | - | - | - | - | **진행 예정 (2순위 - 아키텍처 다변화)** |
| **Exp 13** | **RT-DETR-l Baseline** | Transformer 기반 모델 학습 (CNN 이종결합) | **DETR** | 960 | 50 | - | - | - | - | **진행 예정 (2순위 - 병렬학습)** |
| **Exp 14** | **Hetero WBF** | YOLO11s + YOLO26-n + RT-DETR 결합 | **Mix** | Mix | - | - | - | - | - | **진행 예정 (3순위 - 압도적 점프 유도)** |
| **Exp 15** | **Pseudo-Labeling** | 앙상블 모델 기반 Test 셋 Pseudo-label 생성 재학습 | **Mix** | Mix | - | - | - | - | - | **진행 예정 (3순위 - 데이터 확장)** |
| **Exp 16** | **Colab Pro Boosting** | A100 GPU 활용 고체급(YOLO11m, YOLOv12) 모델 학습 | **Mix** | 1024| 50 | - | - | - | - | **진행 예정 (번외 - 코랩 유료 결제 시)** |
...
### [우선순위 기반] 성능 최적화 후속 전략 (Post-Exp7 Strategy)
...
1.  **🔥 1순위 (재학습 없는 최적화 - 즉시 실행)** 
    *   **NMS 튜닝 (Exp 8)**: 하드코딩된 `conf`, `iou`를 버리고 Validation 셋 기반 최적의 파라미터 동적 탐색.
...
*   **결정**: 이제는 공간적 변형을 극한으로 주는 **실험 6(Rotation)**을 해보기로 결
...
*   **재현**: `python train_yolov8.py --config configs/exp7.yaml`
