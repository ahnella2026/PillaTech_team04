...
### [심층 리포트] NMS vs WBF의 IoU 파라미터 간섭 현상 (Exp 8 vs Exp 9)
Exp 8에서 로컬 검증 기준 최적 밸런스(0.9926)를 보인 **`iou=0.60`** 설정을 Exp 9의 앙상블 파이프라인(Multi-scale NMS + WBF) 전반에 의도적으로 사용했습니다. 이로 인해 도출된 캐글 0.97243의 획득 요인은 파라미터 숫자 하나가 아닌 **이중으로 구성된 아키텍처**에 있습니다.

**1. 1차 허들: 단일 모델 NMS (test_custom.py)에 0.60을 쓴 이유**
*   단일 NMS에서 `iou=0.60`은 기본값 `0.70`보다 박스 억제(Suppression) 조건이 더 공격적입니다(조금만 겹쳐도 삭제). 단일 모델(Exp 8)로는 억울한 삭제(False Negative)가 발생해 캐글 점수가 하락했지만,
*   **WBF 환경을 전제로 할 때는 노이즈 1차 방어벽 역할**을 합니다. 멀티스케일(640, 960, 1024)이 쏟아내는 엄청난 양의 파편화된 예측들을 각 해상도 단계에서 미리 깐깐하게 정리해 주었습니다.

**2. 2차 허들: WBF 병합 (ensemble_wbf.py)에 0.60을 쓴 이유**
*   NMS와 반대로 WBF 병합 기준에서 `iou=0.60`은 `0.70`보다 **쉽게 묶이게 만드는(Merging) 넉넉한 조건**이 됩니다. (60%만 겹쳐도 같은 객체로 인정)
*   1차 NMS를 뚫고 올라온 3개 해상도의 선별된 박스들이 70% 겹침의 깐깐한 허들에 막혀 분할(False Positive)되지 않도록, 0.60의 루즈한 기준을 주어서 하나의 박스로 만들었습니다.

**[결론]**
0.97243을 달성할 수 있었던 이유는, "단일 모델에서는 약간 불리했던 타이트한 NMS 필터링(0.60)이 멀티스케일로 쏟아지는 박스를 넉넉하게 묶어주는 WBF 병합(0.60) 파이프라인 안에서 강력한 시너지를 냈기 때문"입니다.
