...
| ID | 실험명 | 분기 조건 (Split / Augment) | Res | Epoch | **mAP50** | **mAP@50-95** | **mAP@75** | Best Epoch | 비고 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Ref 1**| **Teammate (YOLOv8n)** | Random Split / Default Augs | 640 | 50 | **0.790** | 0.760 | - | - | 예원 레퍼런스 |
| **Exp 0** | **Baseline (YOLOv8n)** | Random Split / Default Augs | 640 | 50 | 0.959 | 0.935 | **0.950** | 50 | 극단적 데이터 누수 발생 |
| **Exp 1** | **Stratified Split** | **Stratified Split** / Default Augs | 640 | 20 | 0.402 | 0.384 | **0.395** | 20 | 계층적 분할 (20 epoch 기준) |
| **Exp 1-2**| **Stratified (Full)** | Stratified Split / Default Augs | 640 | 50 | **0.960** | 0.943 | **0.945** | **36** | 분할만으로는 데이터 누수 완벽 제어 불가 |
| **Exp 2** | **Copy-Paste** | Stratified + **Copy-Paste** | 640 | 50 | - | - | - | - | **예정** |
| **Exp 3** | **Integrated Opt** | Stratified + C&P + **1024px** | 1024| 50 | - | - | - | - | **최종 목표 (>0.90)** |

---

## 🔬 정성적 분석 및 이슈 사항 (Insights & Issues)

### [Exp 0] Baseline (Random Split)
*   **문제점**: mAP가 0.959까지 비정상적으로 높게 측정됨. 이는 유사도가 높은 사진들이 학습/검증 셋에 골고루 섞여 검증 데이터가 유출된(Data Leakage) 결과로 판단됨.
*   **교훈**: 랜덤 분할은 지표를 과하게 낙관적으로 만듦. 실제 캐글(0.701)과의 괴리가 큼.

### [Exp 1] Stratified Split (20 epoch)
*   **분석**: 랜덤 분할 대비 지표가 0.402로 급감함. 이는 희귀 클래스들이 검증 데이터셋에 정직하게 포함되면서 나타나는 "정상적인 지표 하락" 현상임.
*   **결정**: 0.402를 기준점으로 잡고 다음 실험 진행.

### [Exp 1-2] Stratified Split (50 epoch / Full)
*   **현상**: 50 Epoch까지 학습 시 mAP50이 다시 0.96까지 상승함.
