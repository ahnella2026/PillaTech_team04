...
google drive를 통해 공유받은 가중치는 아래 명명 규칙(Tri-Match Policy)에 따라 배치하는 것을 권장합니다.
...
Exp 12 베이스라인과 동일한 성능을 재현하려면 추론 시 아래 파라미터를 반드시 준수하거나 전용 설정을 사용하세요.
...
*   **가중치 공유 방법**: `runs/exp12_train_yolo11s_noflip/weights/best.pt`는 Google Drive에 공유되어 있습니다. 4조 팀원분들은 같은 경로에 배치해주세요. 
...
1.  **설정 상속**: `configs/train/exp12_train_yolo11s_noflip.yaml`을 복사하여 모델 체급(`yolo11m`) 혹은 새로운 아키텍처(RT-DETR 등)로 확장하세요.
2.  **명명 규칙 준수**: `파일명 == YAML 내 name == runs 폴더명`을 일치시켜 실험 계보를 명확히 관리하세요.
3.  **앙상블 전략**: 정제된 Exp 12 가중치를 바탕으로 시드 앙상블 혹은 멀티스케일 추론(`src/ensemble_wbf.py`)을 적용하여 0.99 돌파를 목표로 해봅시다. 
