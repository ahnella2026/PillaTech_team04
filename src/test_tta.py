...
def run_test_and_save_csv():
    ...
    MODEL_PATH = 'runs/yolo_exp4_yolov8s2/weights/best.pt'
    TEST_IMG_DIR = 'data/sprint_ai_project1_data/test_images' 
    OUTPUT_CSV = 'v4_yolov8s_results_tta.csv'
    YAML_PATH = 'data/yolo_dataset_v3_aug/dataset.yaml'
    ...
    for img_name in tqdm(image_files):
        ...
        outputs = model.predict(
            source=img_path, 
            conf=0.25,        
            imgsz=640,        # 이미지 사이즈는 원본 사이즈나, 학습시 인풋으로 들어간 사이즈
            augment=True,     # ⭐ TTA 활성화 (여러 번 보고 평균을 냄)
            device='cpu',     # 원하는 device로 재설정
            verbose=False
        )
